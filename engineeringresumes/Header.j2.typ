...
{% endif %}
#connections(
{% for connection in cv.connections if 'tel:' not in connection and 'mailto:' not in connection %}
  [{{ connection }}],
{% endfor %}
)
#v({{ design.section_titles.space_above }})
#connections(
{% for connection in cv.connections if 'tel:' in connection or 'mailto:' in connection %}
  [{{ connection }}],
{% endfor %}
)
{% if cv.photo %}
